...
with open(input_file, 'r', newline='', encoding='UTF-8') as filereader:
	...
	with open(output_file, 'w', newline='', encoding='UTF-8') as filewriter:
		...
		print(header_list)
        # map(str,header_list)
        # : header_list 리스트를 반복하여 str 문자열로 변환한다(해당 모든 요소)
        # 문자열.join(추가문자열)
   ...
   # 실행하기
   # python .\01_csv파일입출력.py "D:\python\pythonstudy\9일차\CSV\input\sample.csv" "D:\python\pythonstudy\9일차\CSV\output\sample.csv"
   
           #"D:\python\pythonst udy\9일차\CSV\input\sample.csv"
           #"D:\python\pythonstudy\9일차\CSV\output\sample.csv"